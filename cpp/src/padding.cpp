...
int Padding::forward(const Mat& bottom_blob, Mat& top_blob, const Option& opt) const
{
    ...
    if(dims == 3) {
        ...
        for(int q = 0; q < outc; q++) {
            ...
            if(((q < front) || (q >= (channels + front))) && type == 0) {
                ...
                if(type == 2) {
                    q_ = std::abs(q_);
                    q_ = (channels - 1) - std::abs(q_ - (channels - 1));
                }
    ...
    if(dims == 4) {
        ...
        #pragma omp paralle for num_threads(opt.num_threads)
        for(int q = 0; q < channels; q++) {
            ...
            for(int z = 0; z < outd; z++) {
                ...
                if(((z < front) || (z >= (d + front))) && type == 0) {
                    ...
                    if(elemsize == 2) {
                        copy_make_border_image<unsigned char>(m, borderm, top, left, type, support_fp16_storage && opt.use_fp16_storage ? float32_to_float16(pad_value) : float32_to_bfloat16(pad_value));
                    }
